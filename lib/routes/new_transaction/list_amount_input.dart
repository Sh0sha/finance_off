...
import 'package:financeOFF/routes/new_transaction/amount_text.dart';
import 'package:financeOFF/routes/new_transaction/input_amount_sheet/calculator_button.dart';
import 'package:financeOFF/routes/new_transaction/input_amount_sheet/input_value.dart';
...
import 'package:financeOFF/widgets/numpad.dart';
...
class InputAmountSheet extends StatefulWidget {
  ...
  /// Должен ли пользовательский ввод стереть прежнее значение
  final bool overrideInitialAmount;

  ///Если у пользователя есть несколько учетных записей в разных валютах, мы не можем
  //  уверены, какую валютную транзакцию совершает пользователь.
  final bool hideCurrencySymbol;

  ///Будет игнорироваться, если для [lockSign] установлено значение true.
  final bool allowNegative;

  /// отключить переключение между + и -
  final bool lockSign;

  /// Небольшой заголовок над суммой
  final String? title;

  /// Должна ли эта цифровая клавиатура иметь кнопку калькулятора?
  final bool haveCalcul;
...
class _InputAmountSheetState extends State<InputAmountSheet>
  ...
  /// Количество десятичных знаков, используемых для валюты
              ...
              children: [
                ...
                Numpad(
                  children: [
                    ...
                    if (widget.haveCalcul) _doneButton(context),
                  ],
                ),
                const SizedBox(height: 16.0),
              ],
  ...
  /// Возвращает итерацию трех виджетов
  ///
  /// [row] - 0, 1, 2
  ...
  Widget _doneButton(BuildContext context) {
    ...
    return NumpadButton(
      backgroundColor: popOnClick ? context.flowColors.dohod : null,
      crossAxisCellCount: widget.haveCalcul ? 1 : 2,
      onTap: () {
        if (popOnClick) {
          _saveOrPop();
        } else {
  ...
  void removeDigit() {
    // Если пользователь удаляет цифру, он, скорее всего,
    //  чтобы изменить несколько последних чисел
  ...
  void _copy() {
    // Снимите выделение с текста, чтобы избежать путаницы
